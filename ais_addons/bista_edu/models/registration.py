...
class Registration(models.Model):
    ...
    def admission_date_change(self):
        ...
        if self.admission_date and self.batch_id:
            ...
            for month_year in self.batch_id.month_ids.search([('batch_id','=',self.batch_id.id),
                                                              ('leave_month','=',True)]):
    ...
    def compare_with_current_date(self,selected_date):
        ...
        if selected_date:
            ...
            if selected_date.strftime("%d/%m/%Y") < current_date.strftime("%d/%m/%Y"):
                return False
            else:
                return True
...
#       *******************Ministry Approval************************************
    ...
    def create(self, vals):
        ...
        if vals['birth_date'] and vals['course_id'] and vals['batch_id']:
            ...
            if (age_dict['years'] == max_age) and (age_dict['months'] > 0 or age_dict['days'] > 0):
                ...
                template_rec.write({'email_to': res.parent_email, 'email_from': email_sender.smtp_user, 'email_cc': 'Erpemails_ais@iqraeducation.net'})
    ...
    def reg_to_decision_pending(self):
        ...
        if flag=="1":
            ...
            template_rec.write({'email_to': self.email, 'email_from': email_sender.smtp_user, 'email_cc': 'Erpemails_ais@iqraeducation.net'})
    ...
    def confirm_payment(self):
        if self.add_form_link != '' and self.add_form_filled==True:
            self.state='ministry_approval'
            self.student_id.waiting_approval = True
        else:
            raise except_orm(_('Warning!'),
...
#        if self.state_hide=='' or self.state_hide=='enquiry' or self.state_hide=='reg' or self.state_hide=='pending' or self.state_hide=='waiting_list':
    ...
    @api.multi
    def reg_pay_manually(self, journal_id, bank_name=False, chk_num=False, sdate=False, exdate=False, cheque_pay=False,
                         party_name=False):
        """
            ...
            if exist_stu_fee == 0:
                ...
                for student_fee_rec in self.reg_fee_line:
                    if student_fee_rec.amount:
                        ...
                        full_name = str(self.name or '') + ' ' + str(self.middle_name or '') + ' ' + str(
                            self.last_name or '')
                        jounral_dict1.update({'name': full_name, 'debit': student_fee_rec.amount, 'credit': 0.00})
                        jounral_dict2.update({'name': full_name, 'credit': student_fee_rec.amount, 'debit': 0.00,
                ...
                if amount > 0.0 and add_jounral.journal_id.is_cheque:
                    self.create_pdc_detail(add_jounral, amount, bank_name=bank_name, chk_num=chk_num, sdate=sdate,
                                           exdate=exdate, party_name=party_name)
            else:
                self.fee_status = 'reg_fee_pay'
                self.trx_date = datetime.now()
            mail_obj = self.env['mail.mail']
    ...
    def send_payfort_reg_pay_link(self):
        ...
        #
            ...
            for each in self.reg_fee_line:
                ...
                final_amount = str(int(final_amount * 100))
        ...
        if not active_payforts.sha_in_key:
            ...
            enquiry_no) + SHA_Key + 'PSPID=%s'%(PSP_ID) + SHA_Key
            ss = 'AMOUNT=%s&CURRENCY=AED&LANGUAGE=EN_US&ORDERID=%s&PSPID=%s' % (final_amount, enquiry_no,PSP_ID)
            ...
            body_dynamic_html = template_rec.body_html + 'You are requested to pay the (non-refundable) Registration Fee of AED %s.<br/>' \
                                                 '<p><a href=%s><button>Click Here</button> to pay Registration Fee</a></p></div>'%(pay_able_amount,link)
    ...
    def send_payfort_acd_for_next_year(self):
        ...
        for fee_structure_rec in self.student_id.student_fee_line:
            ...
            next_term = False
            if fee_structure_rec.fee_pay_type.name != 'term':
    ...
    def mini_app_to_done(self,args):
        ...
        self.student_id.waiting_approval = False
