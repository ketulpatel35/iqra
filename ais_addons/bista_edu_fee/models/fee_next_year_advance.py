...
class AccounntVoucherInheritAdvance(models.Model):
    ...
    def send_receipt(self):
        ...
        if self.partner_id.is_student == True:
            template_id = ir_model_data.get_object_reference('bista_edu_fee', 'email_template_send_receipt')[1]
            ...
            template_id = ir_model_data.get_object_reference('bista_edu_fee', 'email_template_send_receipt_parent')[1]
            template_rec = self.env['email.template'].sudo().browse(template_id)
            email = self.partner_id.parents_email

        template_rec.write({'email_to': email , 'email_from': email_sender.smtp_user, 'email_cc': 'Erpemails_ais@iqraeducation.net'})
