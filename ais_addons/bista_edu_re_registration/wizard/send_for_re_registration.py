    ...
    def send_re_registration_link(self):
        ...
        if regular_student_rec:
            for student_rec in regular_student_rec:
                joining_date = datetime.strptime(student_rec.admission_date, "%Y-%m-%d")
                if not joining_date:
                    raise except_orm(_('Warning!'),
            ...
            if len(re_reg_children_list) > 0:
                ...
                for parent_rec in parents_list:
                    ...
                    template_id = ir_model_data.get_object_reference('bista_edu_re_registration', 'email_template_re_registration_request')[1]
