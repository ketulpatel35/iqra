    ...
    def resend_tc_form_link(self):
        ...
        for tc_student_rec in tc_obj.browse(active_ids):
            ...
            if tc_student_rec.tc_form_filled != True:
                ...
                template_id = ir_model_data.get_object_reference('bista_transfer_certificate', 'email_template_resend_tc_form_email')[1]
                template_rec = self.env['email.template'].browse(template_id)
                template_rec.write({'email_to' : tc_student_rec.name.parents1_id.parents_email,'email_from':email_sender.smtp_user, 'email_cc': 'Erpemails_ais@iqraeducation.net'})
