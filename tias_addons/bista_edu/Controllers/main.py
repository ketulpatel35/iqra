...
class payfort_payment_status(http.Controller):
    ...
    def resend_academic_fee_payment(self, voucher_rec, amount, env, pay_id):
        ...
        for voucher in voucher_rec:
            # partner_id_list = []
            # if voucher_rec.student_class and voucher_rec.student_section:
            #     for child_rec in voucher_rec.partner_id.chield1_ids:
            #         if voucher_rec.student_class.id == child_rec.class_id.id and voucher_rec.student_section.id == child_rec.section_id.id:
            #             partner_id_list.append(child_rec.id)
            # elif voucher_rec.student_class and not voucher_rec.student_section:
            #     for child_rec in voucher_rec.partner_id.chield1_ids:
            #         if voucher_rec.student_class.id == child_rec.class_id.id:
            #             partner_id_list.append(child_rec.id)
            # elif not voucher_rec.student_class and voucher_rec.student_section:
            #     for child_rec in voucher_rec.partner_id.chield1_ids:
            #         if voucher_rec.student_section.id == child_rec.section_id.id:
            #             partner_id_list.append(child_rec.id)
            # else:
            #     partner_id_list.append(voucher_rec.partner_id.id)
