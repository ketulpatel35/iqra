    ...
    def submit_fee(self):
        ...
        if brw_reg.state=='reg':
            ...
            brw_reg.reg_pay_manually(journal_id,bank_name=bankname,chk_num=chk_num,
                    sdate=sdate,exdate=exdate,cheque_pay=True,party_name=party_name)
                ...
                if not brw_reg.next_year_advance_fee_id:
                    ...
                    if voucher_id:
                        ...
                        if inv_obj.state == 'open':
                            ...
                            template_id = ir_model_data.get_object_reference('bista_edu', 'email_template_academic_fee_receipt_open')[1]
                            template_rec = self.env['email.template'].browse(template_id)
                            template_rec.write({'email_to' : brw_reg.email,'email_from':email_sender.smtp_user})
                    ...
                    if brw_reg.next_year_advance_fee_id.total_amount <= brw_reg.next_year_advance_fee_id.total_paid_amount:
                        brw_reg.next_year_advance_fee_id.state = 'fee_paid'
                        brw_reg.fee_status = 'academy_fee_pay'
                    elif brw_reg.next_year_advance_fee_id.total_paid_amount < brw_reg.next_year_advance_fee_id.total_amount and brw_reg.next_year_advance_fee_id.total_paid_amount != 0.00:
                    ...
                    ir_model_data = self.env['ir.model.data']
                    template_id = \
                    ir_model_data.get_object_reference('bista_edu', 'email_template_academic_fee_receipt_paid')[1]
                    template_rec = self.env['email.template'].browse(template_id)
                    template_rec.write({'email_to': brw_reg.email, 'email_from': email_sender.smtp_user})
                    template_rec.send_mail(voucher_id.id, force_send=True)
            else:
